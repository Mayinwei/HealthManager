//
//  SunSugarDetailViewController.m
//  HealthManager
//
...
@interface SunSugarDetailViewController ()<UITextFieldDelegate,UITableViewDataSource,UITableViewDelegate,MrDatePickerDelegate,SunDataSugarViewDelegate>
...
@implementation SunSugarDetailViewController
...
-(void)loadData
{
    ...
    dic[@"parma"]=[NSString stringWithFormat:@"GetXT*%@*%@ 00:00:00.000,%@ 23:59:59.000*1*10*0",userCode,oldDate,currentDate];
...
-(void)loadOldData
{
    ...
    dic[@"access_token"]=login.access_token;
    self.PageIndex++;
    dic[@"parma"]=[NSString stringWithFormat:@"GetXT*%@*%@ 00:00:00.000,%@ 23:59:59.000*%d*10*0",userCode,oldDate,currentDate,self.PageIndex];
    AFHTTPSessionManager *manager=[AFHTTPSessionManager manager];
    [manager POST:MyURL parameters:dic progress:nil success:^(NSURLSessionDataTask * _Nonnull task, id  _Nullable json) {
        SunErrorModel *errorModel=[SunErrorModel sunErroWithArray:json];
        if (errorModel!=nil) {
        ...
        NSMutableArray *oldArray=[SunSugarDetailModel mj_objectArrayWithKeyValuesArray:json];
...
-(void)sugarViewPlaySound:(SunDataSugarView *)sugar
{
    ...
    SunSugarDetailModel *third=[self.arrayData objectAtIndex:index];
...
{
    SunSugarTableViewCell *cell=[SunSugarTableViewCell cellWithTableView:tableView];
    SunDataSugarView *sugarView=(SunDataSugarView *)cell.sugar;
    sugarView.delegate=self;
    sugarView.soundButton.tag=indexPath.row;
    cell.sugarDetail=self.arrayData[indexPath.row];
    return cell;
}
//设置行高
